...
def login(nomor):
  global client
  api_id = 3313445
  api_hash = 'c04b3ac20bb914367e9f9346047af6df'
  phone_number = nomor
  ...
  if posts.messages[0].message.find("Sorry, there are no new ads available") != -1:
    ...
    try:
     ...
     if response.status_code == 503:
         infile = open('link.txt', 'r').readlines()
         with open('link.txt', 'w') as outfile:
     ...
     elif soup.find("div",class_="g-recaptcha") is None and soup.find('div', id="headbar") is None:
        sleep(2)
        posts = client(GetHistoryRequest(peer=channel_entity,limit=1,offset_date=None,offset_id=0,max_id=0,min_id=0,add_offset=0,hash=0))
        message = posts.messages[0].message
        if posts.messages[0].message.find("You must stay") != -1 or posts.messages[0].message.find("Please stay on") != -1:
     ...
     elif soup.find("div",class_="g-recaptcha") is not None:
      sys.stdout.write("\r")
      sys.stdout.write("                                                                ")
      sys.stdout.write("\r")
      ...
      if posts.messages[0].message.find("You must stay") != -1 or posts.messages[0].message.find("Please stay on") != -1:
          ...
          sys.stdout.write("\r                                                              \r")
          sys.stdout.write(f"\r{abu2}[{red2}x{abu2}]{red} Tidak Bisa Menemukan")
